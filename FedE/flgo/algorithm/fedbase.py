...
class BasicServer(BasicParty):
    ...
    def __init__(self, option={}):
        ...
        self.model = None
        # self.model = BertModel.from_pretrained('/home/fedrag/bge-en')
        self.clients = []
        # basic configuration
        ...
        self.sample_option = 'full'  #设置为full
    ...
    def aggregate(self, model_old, models: list, *args, **kwargs):
        ...
        for name, param in model_old.named_parameters():
            for encrypted_gradients in models:
                if param.grad is not None:
                    encrypted_grad = encrypted_gradients[name]
                    decrypted_grad = encrypted_grad.decrypt()  # 解密梯度
                    decrypted_grad_tensor = torch.tensor(decrypted_grad).view(param.grad.shape)  # 转换为张量
